...
export const Tabs: React.FC<TabsProps> = (props) => {
  ...
  const getTabListStyle = () => {
    const style: React.CSSProperties = {
      borderColor: `var(--blox-color-${variant}-200, var(--blox-tabs-border-color, #bae6fd))`,
    };
  ...
  const getTabStyle = (item: TabItem) => {
    ...
    // Base style
    const style: React.CSSProperties = {
      color: isActive ? `var(--blox-color-${variant}-700, var(--blox-tabs-active-color, #0369a1))` : isDisabled ? `var(--blox-tabs-disabled-color, #9ca3af)` : `var(--blox-color-${variant}-600, var(--blox-tabs-color, #0284c7))`,
    ...
    if (variantStyle === "enclosed" && isActive) {
      ...
      if (orientation === "horizontal") {
        style.borderColor = `var(--blox-color-${variant}-200, var(--blox-tabs-border-color, #bae6fd))`;
        style.borderWidth = "1px 1px 0 1px";
      ...
      if (isActive) {
        style.backgroundColor = `var(--blox-color-${variant}-100, var(--blox-tabs-soft-active-bg, #e0f2fe))`;
      }
    } else if (variantStyle === "rounded") {
      style.border = `1px solid ${isActive ? `var(--blox-color-${variant}-400, var(--blox-tabs-rounded-active-border, #38bdf8))` : "transparent"}`;
      if (isActive) {
  ...
  const getIndicatorStyle = () => {
    return {
      backgroundColor: `var(--blox-color-${variant}-500, var(--blox-tabs-indicator-color, #0ea5e9))`,
  ...
  const renderTab = (item: TabItem, index: number) => {
    ...
    let tabClasses = "flex items-center py-2 px-4 font-medium outline-none focus:ring-2 focus:ring-offset-2";
