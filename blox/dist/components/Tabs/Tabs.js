...
export const Tabs = (props) => {
    ...
    const config = getComponentConfig("Tabs");
    ...
    const [selectedTab, setSelectedTab] = useState(activeTab || defaultActiveTab || (items.length > 0 ? items[0].id : ""));
    ...
    const activeTabContent = (_a = items.find((item) => item.id === selectedTab)) === null || _a === void 0 ? void 0 : _a.content;
    // Base classes
    const baseClasses = "";
    // Orientation classes
    ...
    const renderTab = (item, index) => {
        ...
        return (_jsxs("button", { id: `tab-${item.id}`, role: "tab", "aria-selected": isActive, "aria-controls": `tabpanel-${item.id}`, tabIndex: isActive ? 0 : -1, disabled: isDisabled, onClick: () => !isDisabled && handleTabChange(item.id), className: tabClasses, style: getTabStyle(item), ref: (el) => (tabsRef.current[item.id] = el), children: [item.icon && _jsx("span", { className: "mr-2", children: item.icon }), item.label] }, item.id));
    };
    // Render the tab indicator for line and enclosed variants
    const renderIndicator = () => {
        if (variantStyle !== "line" && variantStyle !== "enclosed")
            return null;
        const indicatorClasses = orientation === "horizontal" ? "absolute bottom-0 h-0.5" : "absolute left-0 w-0.5";
        return (_jsx("div", { ref: indicatorRef, className: indicatorClasses, style: getIndicatorStyle() }));
    };
    // Render tab panels
    const renderTabPanels = () => {
        return (_jsx("div", { className: `blox-tab-panels ${tabPanelClassName}`, children: alwaysRenderContent ? (items.map((item) => (_jsx("div", { id: `tabpanel-${item.id}`, role: "tabpanel", "aria-labelledby": `tab-${item.id}`, hidden: item.id !== selectedTab, className: item.id === selectedTab ? "block" : "hidden", children: item.content }, item.id)))) : (_jsx("div", { id: `tabpanel-${selectedTab}`, role: "tabpanel", "aria-labelledby": `tab-${selectedTab}`, children: activeTabContent })) }));
    };
    return (_jsxs("div", { className: `${baseClasses} ${orientationClasses[orientation]} ${className}`, style: getContainerStyle(), ...rest, children: [_jsxs("div", { className: `blox-tab-list relative ${orientation === "horizontal" ? "flex" : "flex-col"} ${tabListClassName}`, role: "tablist", "aria-orientation": orientation, style: getTabListStyle(), children: [leftContent && _jsx("div", { className: "flex items-center mr-2", children: leftContent }), items.map(renderTab), rightContent && _jsx("div", { className: "flex items-center ml-2", children: rightContent }), renderIndicator()] }), _jsx("div", { className: orientation === "horizontal" ? "pt-4" : "pl-4", children: renderTabPanels() })] }));
};
